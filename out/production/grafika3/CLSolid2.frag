...
//    vec3 nNormala = textureBricksn.xyz;
    ...
    vec3 viewDir = normalize(normalize(viewPositionCLSolid2) - position.rgb);
    vec3 reflectDir = reflect(lightCLSolid2, nNormala);
    vec3 lightDir = normalize(lightCLSolid2 - position.rgb);
    float diff = max(dot(nNormala, lightDir),0.0);
    float  attenuation = clamp( 10.0 /distance(lightCLSolid2,vec3(position.xyz)), 0.0, 1.0);
    ...
    switch(surfaceCL2) {
        ...
        case  6: //vzdalenost od svetla
            float dist =  distance(vec3(position.xyz), normalize(lightCLSolid2));
